...
function dirigent(_div, _instructions, _plot_width) {
  ...
  const scales_bindings = scales.make_scales_to_bindings(_instructions['bindings'], _instructions['data'], dimensions)
  let extended_instructions = preps.prepare_extended_instructions(_instructions, scales_bindings)

  // Make plot panel (excl. margins)
  svg = panel.plot_panel(svg, scales_bindings['x'], scales_bindings['y'], dimensions);
  
  // Add axis
  svg = axis.plot_axis(svg, scales_bindings['x'], scales_bindings['y'], dimensions,  extended_instructions["bindings"]["x"],  extended_instructions["bindings"]["y"]);
